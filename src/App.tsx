...
import { TipSelect } from "./components/tip-select/tip-select.component";
...
function App() {
  ...
  function validationBill(e: React.KeyboardEvent) {
    e = e || window.event;
  ...
  function validationPeople(e: React.KeyboardEvent) {
    e = e || window.event;
    const charCode = typeof e.which == "undefined" ? e.keyCode : e.which;
    const charStr = String.fromCharCode(charCode);
  ...
  function changeBill(e: React.ChangeEvent<HTMLInputElement>) {
    if (+e.target.value == 0 && e.target.parentElement) {
      e.target.parentElement.style.outline = "1px solid red";
      setZeroBill(true);
      return;
    }
    initialContext.bill = +e.target.value;
    setBill(+e.target.value);
    setZeroBill(false);
  }
  function changePeople(e: React.ChangeEvent<HTMLInputElement>) {
    if (+e.target.value == 0 && e.target.parentElement) {
      e.target.parentElement.style.outline = "1px solid red";
      setZeroPeople(true);
      return;
    }
    initialContext.numPeople = +e.target.value;
    setNumPeople(+e.target.value);
    setZeroPeople(false);
  }
  function addOutline(e: React.FocusEvent<HTMLInputElement>) {
    if (!e.target.parentElement) return;

    e.target.parentElement.style.outline = "1px solid var(--primary-color)";
  }
  function removeOutline(e: React.FocusEvent<HTMLInputElement>) {
    if (!e.target.parentElement || zeroBill) return;
  ...
  return (
    <div className="App">
      ...
      <TipDataContext.Provider value={initialContext}>
        <main>
          <section>
            <form action="#">
              ...
              <div className="form-box people">
                ...
                <div className="input">
                  ...
                  <input
                    ...
                    placeholder="0"
                    onKeyPress={(e) => validationPeople(e)}
